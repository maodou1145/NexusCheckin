...
> 这份是**工程笔记**（开发过程、踩坑记录、Lite Wearable 约束、调试方法），面向维护者。
> 想了解「这应用是干什么的、怎么装怎么用」，请看 [README.md](README.md)。
...
> 空出的三屏改为与每日一句同款的**每日内容栏目**：每日诗词（今日诗词 API）、
> 历史上的今天（60s API）、每日英语（内置词库按日期轮换，零请求）。

---

## 一、它现在是什么

竖向 `swiper` **六屏**，上下滑动换屏；屏内按钮随屏滑动，各自绑 `onclick`：

| 屏 | 内容 | 按钮 | 数据来源 |
|---|---|---|---|
| 1/6 | 每日签到 + 幸运大转盘 | 立即签到 / 开始抽奖 | `GET/POST /api/points/*` |
| 2/6 | 每日一句 | 换一句 / **查看详情** | 一言 API `v1.hitokoto.cn`（公开免费） |
| 3/6 | 每日诗词 | 换一首 / **查看详情** | 今日诗词 `v1.jinrishici.com/all.json`（162B，公开免费） |
| 4/6 | 历史上的今天 | 换一批（3 条一组）/ **点条目看完整详情** | 60s API `60s-api.viki.moe/v2/today_in_history`（约 5.7KB） |
| 5/6 | 每日英语 | 换一个 / **查看详情** | 词源：rawfile/words.json 120 词按日期轮换（+ 内置 40 词兜底）；**词条内容（音标/中文释义/双语例句）实时取自有道词典 API**，失败回落本地字段 |
...
- 头像样式：**80×80 圆形**（`.avatar` border-radius 40px，lite 只认 length 不认百分比；问候区高度同步 48→88）
